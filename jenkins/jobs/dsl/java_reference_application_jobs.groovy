...
def workspaceFolderName = "daas_workspace"
//def projectFolderName = "${PROJECT_NAME}"
def projectFolderName = "daas_project"
...
def referenceAppGitUrl = "https://github.com/Aditazz/daas-demo-app.git"
def regressionTestGitUrl = "https://github.com/Aditazz/daas-demo-app.git"


// Jobs
def buildAppJob = freeStyleJob(projectFolderName + "/DaaS_Build")
def unitTestJob = freeStyleJob(projectFolderName + "/DaaS_Unit_Tests")
def codeAnalysisJob = freeStyleJob(projectFolderName + "/DaaS_Code_Analysis")
def deployJob = freeStyleJob(projectFolderName + "/DaaS_Deploy")
def regressionTestJob = freeStyleJob(projectFolderName + "/DaaS_Regression_Tests")
def performanceTestJob = freeStyleJob(projectFolderName + "/DaaS_Performance_Tests")
def deployJobToProdA = freeStyleJob(projectFolderName + "/DaaS_Deploy_ProdA")
def deployJobToProdB = freeStyleJob(projectFolderName + "/DaaS_Deploy_ProdB")

// Views
def pipelineView = buildPipelineView(projectFolderName + "/Java_DaaS")

pipelineView.with {
    title('Reference Application Pipeline')
    displayedBuilds(5)
    selectedJob(projectFolderName + "/DaaS_Build")
...
buildAppJob.with {
    ...
    scm {
        git {
            remote {
                ...
                credentials("franciscoaditazz_ssh")
            }
            branch("*/master")
        }
    }
    ...
    publishers {
        ...
        downstreamParameterized {
            trigger(projectFolderName + "/DaaS_Unit_Tests") {
...
unitTestJob.with {
    ...
    parameters {
        ...
        stringParam("PARENT_BUILD", "DaaS_Build", "Parent build name")
    }
    wrappers {
        preBuildCleanup()
        injectPasswords()
    ...
    publishers {
        archiveArtifacts("**/*")
        downstreamParameterized {
            trigger(projectFolderName + "/DaaS_Code_Analysis") {
                condition("UNSTABLE_OR_BETTER")
                parameters {
                    predefinedProp("B", '${B}')
...
sonar.scm.enabled=false''')
    ...
    publishers {
        downstreamParameterized {
            trigger(projectFolderName + "/DaaS_Deploy") {
                condition("UNSTABLE_OR_BETTER")
                parameters {
                    predefinedProp("B", '${B}')
                    predefinedProp("PARENT_BUILD", '${PARENT_BUILD}')
...
deployJob.with {
    ...
    steps {
        copyArtifacts("DaaS_Build") {
            buildSelector {
                buildNumber('${B}')
                includePatterns('target/spring-poc-0.0.1-SNAPSHOT.war')
            }
        }
        shell('''set +x
            |export SERVICE_NAME="$(echo ${PROJECT_NAME} | tr '/' '_')_${ENVIRONMENT_NAME}"
            |docker cp ${WORKSPACE}/target/spring-poc-0.0.1-SNAPSHOT.war  ${SERVICE_NAME}:/usr/local/tomcat/webapps/
    ...
    publishers {
        downstreamParameterized {
            trigger(projectFolderName + "/DaaS_Regression_Tests") {
                condition("UNSTABLE_OR_BETTER")
                parameters {
                    predefinedProp("B", '${B}')
                    predefinedProp("PARENT_BUILD", '${PARENT_BUILD}')
...
regressionTestJob.with {
    ...
    steps {
        shell('''
            ...
            |JOB_WORKSPACE_PATH="/var/lib/docker/volumes/jenkins_slave_home/_data/${PROJECT_NAME}/DaaS_Regression_Tests"
    ...
    publishers {
        downstreamParameterized {
            trigger(projectFolderName + "/DaaS_Performance_Tests") {
                condition("UNSTABLE_OR_BETTER")
                parameters {
                    predefinedProp("B", '${B}')
                    predefinedProp("PARENT_BUILD", '${PARENT_BUILD}')
...
performanceTestJob.with {
    ...
    parameters {
        stringParam("B", '', "Parent build number")
        stringParam("PARENT_BUILD", "DaaS_Regression_Tests", "Parent build name")
        stringParam("ENVIRONMENT_NAME", "CI", "Name of the environment.")
    }
    wrappers {
        preBuildCleanup()
    ...
    publishers {
        ...
        buildPipelineTrigger(projectFolderName + "/DaaS_Deploy_ProdA") {
            parameters {
                predefinedProp("B", '${B}')
                predefinedProp("PARENT_BUILD", '${PARENT_BUILD}')
            }
...
deployJobToProdA.with {
    ...
    publishers {
        buildPipelineTrigger(projectFolderName + "/DaaS_Deploy_ProdB") {
            parameters {
                predefinedProp("B", '${B}')
                predefinedProp("PARENT_BUILD", '${PARENT_BUILD}')
