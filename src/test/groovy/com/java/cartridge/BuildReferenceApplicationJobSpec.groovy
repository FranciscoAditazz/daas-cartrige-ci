...
class BuildReferenceApplicationJobSpec extends Specification {
    ...
    @Shared
    def Node node = new XmlParser().parseText(helper.jm.savedConfigs["${helper.projectName}/DaaS_Build"])

    def 'DaaS_Build job is exists'() {
        expect:
            helper.jm.savedConfigs[jobName] != null

        where:
            jobName = "${helper.projectName}/DaaS_Build"
    }
    ...
    def 'downstream parameterized trigger on "#triggerJobName" job exists'() {
        ...
        where:
            triggerJobName = "${helper.projectName}/DaaS_Unit_Tests"
    }
