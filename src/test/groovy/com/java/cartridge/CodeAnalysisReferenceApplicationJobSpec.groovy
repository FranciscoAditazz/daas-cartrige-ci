...
/**
 * Tests that JavaReferenceApplication/DaaS_Code_Analysis job works as expected.
 */
class CodeAnalysisReferenceApplicationJobSpec extends Specification {
    ...
    @Shared
    def Node node = new XmlParser().parseText(helper.jm.savedConfigs["${helper.projectName}/DaaS_Code_Analysis"])

    def 'DaaS_Code_Analysis job is exists'() {
        expect:
            helper.jm.savedConfigs[jobName] != null

        where:
            jobName = "${helper.projectName}/DaaS_Code_Analysis"
    }
    ...
    def 'job parameters, "PARENT_BUILD" string parameter with default value "DaaS_Build" exists'() {
        expect:
            node.properties['hudson.model.ParametersDefinitionProperty']['parameterDefinitions'].size() == 1
    ...
    def 'Copy Artifacts project specified on "#jenkinsJobName" jenkins job'() {
        ...
        where:
            jenkinsJobName = 'DaaS_Unit_Tests'
    }
    ...
    def 'downstream parameterized trigger on "#triggerJobName" job exists'() {
        ...
        where:
            triggerJobName = "${helper.projectName}/DaaS_Deploy"
    }
