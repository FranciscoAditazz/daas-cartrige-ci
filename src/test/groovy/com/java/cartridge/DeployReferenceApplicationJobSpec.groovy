...
/**
 * Tests that JavaReferenceApplication/DaaS_Deploy job works as expected.
 */
class DeployReferenceApplicationJobSpec extends Specification {
    ...
    @Shared
    def Node node = new XmlParser().parseText(helper.jm.savedConfigs["${helper.projectName}/DaaS_Deploy"])

    def 'DaaS_Deploy job is exists'() {
        expect:
            helper.jm.savedConfigs[jobName] != null

        where:
            jobName = "${helper.projectName}/DaaS_Deploy"
    }
    ...
    def 'downstream parameterized trigger on "#triggerJobName" job exists'() {
        ...
        where:
            triggerJobName = "${helper.projectName}/DaaS_Regression_Tests"
    }
