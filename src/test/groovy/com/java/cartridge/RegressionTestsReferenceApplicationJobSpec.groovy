...
/**
 * Tests that JavaReferenceApplication/DaaS_Regression_Tests job works as expected.
 */
class RegressionTestsReferenceApplicationJobSpec extends Specification {
    ...
    @Shared
    def Node node = new XmlParser().parseText(helper.jm.savedConfigs["${helper.projectName}/DaaS_Regression_Tests"])

    def 'DaaS_Regression_Tests job is exists'() {
        expect:
            helper.jm.savedConfigs[jobName] != null

        where:
            jobName = "${helper.projectName}/DaaS_Regression_Tests"
    }
    ...
    def 'downstream parameterized trigger on "#triggerJobName" job exists'() {
        ...
        where:
            triggerJobName = "${helper.projectName}/DaaS_Performance_Tests"
    }
