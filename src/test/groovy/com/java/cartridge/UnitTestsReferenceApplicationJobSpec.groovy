...
/**
 * Tests that JavaReferenceApplication/DaaS_Unit_Tests job works as expected.
 */
class UnitTestsReferenceApplicationJobSpec extends Specification {
    ...
    @Shared
    def Node node = new XmlParser().parseText(helper.jm.savedConfigs["${helper.projectName}/DaaS_Unit_Tests"])

    def 'DaaS_Unit_Tests job is exists'() {
        expect:
            helper.jm.savedConfigs[jobName] != null

        where:
            jobName = "${helper.projectName}/DaaS_Unit_Tests"
    }
    ...
    def 'downstream parameterized trigger on "#triggerJobName" job exists'() {
        ...
        where:
            triggerJobName = "${helper.projectName}/DaaS_Code_Analysis"
    }
